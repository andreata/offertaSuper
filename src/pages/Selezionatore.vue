<template>
    <Layout>
        <div>
            <div class="full-container bg-home-2">
                <div class="container">
                    <h1 class="title-home desk">Anziché sbuffare davanti alla lista infinita<br> di tariffe e offerte disponibili,<br> affidati al nostro selezionatore. <br>Ti
                    facciamo risparmiare tempo e denaro.</h1>
                    <p class="p-home desk">Se vuoi iniziare a risparmiare, scopri gratuitamente con il nostro selezionatore quali sono le offerte per le utenze
                    attive oggi più economiche.
                    Ti garantiamo un risparmio di tempo, soldi e pazienza!<br>Compila il form qui in basso inserendo tutti i dati richiesti, attendi qualche minuto e riceverai via e-mail la nostra
                    proposta con le offerte luce e gas, fibra e internet casa, più adatte a te.<br>
</p>
                   

                    <div class="mobl">
                        <div class="home-layout-mobile">
                            <div>
                                <h1 class="title-home-mobile mobl">Affidati al nostro selezionatore</h1>
                                <p class="p-home-mobile mobl">Ti facciamo risparmiare tempo e denaro.</p>
                            </div>
                            <g-image class="img-hero" src="~/assets/images/icon/hero-landing.svg" alt="" width="300" />


                        </div>
                        <div class="bullet-landing">
                            <g-image src="~/assets/images/icon/circle-check-solid.svg" alt="" width="50" />
                            <p>Miglior Prezzo</p>
                        </div>
                        <div class="bullet-landing">
                            <g-image src="~/assets/images/icon/circle-check-solid.svg" alt="" width="50" />
                            <p>Miglior servizio</p>
                        </div>
                        <div class="bullet-landing">
                            <g-image src="~/assets/images/icon/circle-check-solid.svg" alt="" width="50" />
                            <p>Miglior assistenza</p>
                        </div>
                    </div>

                    <form-landing />

                </div>
            </div>


            <!-- Copy e Immagine -->
            
          
            <div class="full-container">
            <div class="container-box desk container">
                <div class="item-box">
                    <p class="promo">Promo del mese</p>
                    <g-image class="img-promo" src="~/assets/images/icon/internet-casa.svg" alt="" width="66" />
                    <h2 class="h2-promo">INTERNET CASA</h2>
                    <h3 class="h3-promo">per una connessione internet casa super veloce</h3>
                    <p class="li-promo"><svg xmlns="http://www.w3.org/2000/svg" width="17.001" height="16.999"
                            viewBox="0 0 17.001 16.999">
                            <path id="Tracciato_760" data-name="Tracciato 760"
                                d="M8.5,17c-.087,0-.175,0-.262,0A8.5,8.5,0,0,1,7.716.034C7.967.012,8.223,0,8.475,0a8.524,8.524,0,0,1,8.491,9.283A8.5,8.5,0,0,1,8.5,17ZM4.49,7.343a.806.806,0,0,0-.225.032A.811.811,0,0,0,3.929,8.74L6.9,11.575a.81.81,0,0,0,1.129,0L13.066,6.59h.005a.809.809,0,0,0-1.138-1.151L7.449,9.869l-2.4-2.3A.808.808,0,0,0,4.49,7.343Z"
                                transform="translate(0 0)" fill="#f9b666" />
                        </svg> <span>risparmi 200€ l’anno</span></p>
            
                    <p class="li-promo"><svg xmlns="http://www.w3.org/2000/svg" width="17.001" height="16.999"
                            viewBox="0 0 17.001 16.999">
                            <path id="Tracciato_760" data-name="Tracciato 760"
                                d="M8.5,17c-.087,0-.175,0-.262,0A8.5,8.5,0,0,1,7.716.034C7.967.012,8.223,0,8.475,0a8.524,8.524,0,0,1,8.491,9.283A8.5,8.5,0,0,1,8.5,17ZM4.49,7.343a.806.806,0,0,0-.225.032A.811.811,0,0,0,3.929,8.74L6.9,11.575a.81.81,0,0,0,1.129,0L13.066,6.59h.005a.809.809,0,0,0-1.138-1.151L7.449,9.869l-2.4-2.3A.808.808,0,0,0,4.49,7.343Z"
                                transform="translate(0 0)" fill="#f9b666" />
                        </svg> <span>offerte senza vincoli</span></p>
            
                    <p class="li-promo"><svg xmlns="http://www.w3.org/2000/svg" width="17.001" height="16.999"
                            viewBox="0 0 17.001 16.999">
                            <path id="Tracciato_760" data-name="Tracciato 760"
                                d="M8.5,17c-.087,0-.175,0-.262,0A8.5,8.5,0,0,1,7.716.034C7.967.012,8.223,0,8.475,0a8.524,8.524,0,0,1,8.491,9.283A8.5,8.5,0,0,1,8.5,17ZM4.49,7.343a.806.806,0,0,0-.225.032A.811.811,0,0,0,3.929,8.74L6.9,11.575a.81.81,0,0,0,1.129,0L13.066,6.59h.005a.809.809,0,0,0-1.138-1.151L7.449,9.869l-2.4-2.3A.808.808,0,0,0,4.49,7.343Z"
                                transform="translate(0 0)" fill="#f9b666" />
                        </svg> <span>attivazione online</span></p>
            
            
            
                    <g-link to="/fibra-la-migliore-offerta-per-il-mercato-italiano/" class="button-pieno">Maggiori
                        informazioni</g-link>
            
                </div>
            
                <div class="item-box">
                    <p class="promo">Risparmia ora</p>
                    <g-image class="img-promo" src="~/assets/images/icon/luce.svg" alt="" width="66" />
                    <h2 class="h2-promo">Luce e gas</h2>
                    <h3 class="h3-promo">per non avere costi eccessivi in bolletta</h3>
                    <p class="li-promo"><svg xmlns="http://www.w3.org/2000/svg" width="17.001" height="16.999"
                            viewBox="0 0 17.001 16.999">
                            <path id="Tracciato_760" data-name="Tracciato 760"
                                d="M8.5,17c-.087,0-.175,0-.262,0A8.5,8.5,0,0,1,7.716.034C7.967.012,8.223,0,8.475,0a8.524,8.524,0,0,1,8.491,9.283A8.5,8.5,0,0,1,8.5,17ZM4.49,7.343a.806.806,0,0,0-.225.032A.811.811,0,0,0,3.929,8.74L6.9,11.575a.81.81,0,0,0,1.129,0L13.066,6.59h.005a.809.809,0,0,0-1.138-1.151L7.449,9.869l-2.4-2.3A.808.808,0,0,0,4.49,7.343Z"
                                transform="translate(0 0)" fill="#f9b666" />
                        </svg> <span>prezzi bloccati</span></p>
            
                    <p class="li-promo"><svg xmlns="http://www.w3.org/2000/svg" width="17.001" height="16.999"
                            viewBox="0 0 17.001 16.999">
                            <path id="Tracciato_760" data-name="Tracciato 760"
                                d="M8.5,17c-.087,0-.175,0-.262,0A8.5,8.5,0,0,1,7.716.034C7.967.012,8.223,0,8.475,0a8.524,8.524,0,0,1,8.491,9.283A8.5,8.5,0,0,1,8.5,17ZM4.49,7.343a.806.806,0,0,0-.225.032A.811.811,0,0,0,3.929,8.74L6.9,11.575a.81.81,0,0,0,1.129,0L13.066,6.59h.005a.809.809,0,0,0-1.138-1.151L7.449,9.869l-2.4-2.3A.808.808,0,0,0,4.49,7.343Z"
                                transform="translate(0 0)" fill="#f9b666" />
                        </svg> <span>contratti chiari</span></p>
            
                    <p class="li-promo"><svg xmlns="http://www.w3.org/2000/svg" width="17.001" height="16.999"
                            viewBox="0 0 17.001 16.999">
                            <path id="Tracciato_760" data-name="Tracciato 760"
                                d="M8.5,17c-.087,0-.175,0-.262,0A8.5,8.5,0,0,1,7.716.034C7.967.012,8.223,0,8.475,0a8.524,8.524,0,0,1,8.491,9.283A8.5,8.5,0,0,1,8.5,17ZM4.49,7.343a.806.806,0,0,0-.225.032A.811.811,0,0,0,3.929,8.74L6.9,11.575a.81.81,0,0,0,1.129,0L13.066,6.59h.005a.809.809,0,0,0-1.138-1.151L7.449,9.869l-2.4-2.3A.808.808,0,0,0,4.49,7.343Z"
                                transform="translate(0 0)" fill="#f9b666" />
                        </svg> <span>attivazione online</span></p>
            
            
            
                    <g-link to="/offerte-luce-e-gas/" class="button-pieno">Risparmia subito</g-link>
            
                </div>
        
            
                <div class="item-box">
                    <p class="promo">Consulenza</p>
                    <g-image class="img-promo" src="~/assets/images/icon/risparmio.svg" alt="" width="66" height="62" />
                    <h2 class="h2-promo">IL SELEZIONATORE</h2>
                    <h3 class="h3-promo">per conoscere tutte le offerte più vantaggiose oggi</h3>
                    <p class="li-promo"><svg xmlns="http://www.w3.org/2000/svg" width="17.001" height="16.999"
                            viewBox="0 0 17.001 16.999">
                            <path id="Tracciato_760" data-name="Tracciato 760"
                                d="M8.5,17c-.087,0-.175,0-.262,0A8.5,8.5,0,0,1,7.716.034C7.967.012,8.223,0,8.475,0a8.524,8.524,0,0,1,8.491,9.283A8.5,8.5,0,0,1,8.5,17ZM4.49,7.343a.806.806,0,0,0-.225.032A.811.811,0,0,0,3.929,8.74L6.9,11.575a.81.81,0,0,0,1.129,0L13.066,6.59h.005a.809.809,0,0,0-1.138-1.151L7.449,9.869l-2.4-2.3A.808.808,0,0,0,4.49,7.343Z"
                                transform="translate(0 0)" fill="#f9b666" />
                        </svg> <span>Ti ascoltiamo per capire le tue esigenze</span></p>
            
                    <p class="li-promo"><svg xmlns="http://www.w3.org/2000/svg" width="17.001" height="16.999"
                            viewBox="0 0 17.001 16.999">
                            <path id="Tracciato_760" data-name="Tracciato 760"
                                d="M8.5,17c-.087,0-.175,0-.262,0A8.5,8.5,0,0,1,7.716.034C7.967.012,8.223,0,8.475,0a8.524,8.524,0,0,1,8.491,9.283A8.5,8.5,0,0,1,8.5,17ZM4.49,7.343a.806.806,0,0,0-.225.032A.811.811,0,0,0,3.929,8.74L6.9,11.575a.81.81,0,0,0,1.129,0L13.066,6.59h.005a.809.809,0,0,0-1.138-1.151L7.449,9.869l-2.4-2.3A.808.808,0,0,0,4.49,7.343Z"
                                transform="translate(0 0)" fill="#f9b666" />
                        </svg> <span>Cerchiamo le soluzioni più convenienti</span></p>
            
            
            
                    <p class="li-promo"><svg xmlns="http://www.w3.org/2000/svg" width="17.001" height="16.999"
                            viewBox="0 0 17.001 16.999">
                            <path id="Tracciato_760" data-name="Tracciato 760"
                                d="M8.5,17c-.087,0-.175,0-.262,0A8.5,8.5,0,0,1,7.716.034C7.967.012,8.223,0,8.475,0a8.524,8.524,0,0,1,8.491,9.283A8.5,8.5,0,0,1,8.5,17ZM4.49,7.343a.806.806,0,0,0-.225.032A.811.811,0,0,0,3.929,8.74L6.9,11.575a.81.81,0,0,0,1.129,0L13.066,6.59h.005a.809.809,0,0,0-1.138-1.151L7.449,9.869l-2.4-2.3A.808.808,0,0,0,4.49,7.343Z"
                                transform="translate(0 0)" fill="#f9b666" />
                        </svg> <span>Selezioniamo i pacchetti più in linea conte</span></p>
            
                    <g-link to="/selezionatore" class="button-pieno">Clicca qui</g-link>
            
                </div>
            </div>
            
            
            <div class="container-box mobl">
                <g-link to="/fibra-la-migliore-offerta-per-il-mercato-italiano/" class="link-no-deco">
                    <div class="item-box">
                        <p class="promo">Promo del mese</p>
                        <div class="flex-homeland">
                            <g-image class="img-promo" src="~/assets/images/icon/internet-casa.svg" alt="" width="66" height="62" />
                            <div>
                                <h2 class="h2-promo">Internet Casa</h2>
                                <h3 class="h3-promo">risparmi 200€ l’anno</h3>
                            </div>
                        </div>
            
                    </div>
                </g-link>
            
                <g-link to="/offerte-luce-e-gas/" class="link-no-deco">
                    <div class="item-box">
                        <p class="promo">Risparmia ora</p>
                        <div class="flex-homeland">
                            <g-image class="img-promo" src="~/assets/images/icon/luce.svg" alt="" width="66" height="62" />
                            <div>
                                <h2 class="h2-promo">Luce e gas</h2>
                                <h3 class="h3-promo">prezzi bloccati</h3>
                            </div>
                        </div>
                    </div>
                </g-link>
            
                <g-link to="/selezionatore/" class="link-no-deco">
                    <div class="item-box">
                        <p class="promo">Consulenza</p>
                        <div class="flex-homeland">
                            <g-image class="img-promo" src="~/assets/images/icon/offerte.svg" alt="" width="66" height="62" />
                            <div>
                                <h2 class="h2-promo">Il selezionatore</h2>
                                <h3 class="h3-promo">Ti ascoltiamo per capire le tue esigenze</h3>
                            </div>
                        </div>
                    </div>
                </g-link>
      
            </div>
            
            
            
            </div>





            <!-- Box Salmone con 5 icone -->

            <!-- Box Salmone con 5 icone -->
            
            <div class="full-container container-salmone">
                <div class="container">
                    <h2 class="text-center mb-80">
                        Perché scegliere Offerta Super?
                    </h2>
                    <div class="grid-five-column">
            
                        <!-- <div>
                          <g-image class="img-salmone" src="~/assets/images/icon/analisi.svg" alt="" width="60" />
                          <div>
                            <h3 class="orange-font">Analisi</h3>
                            <p class="grey-font">Nulla vitae elit libero, a pharetra augue. Aenean eu leo quam. Nulla vitae elit
                              libero, a pharetra augue. </p>
                          </div>
                        </div> -->
            
                        <div>
                            <g-image class="img-salmone" src="~/assets/images/icon/confronto.svg" alt="" width="60" />
                            <div>
                                <h3 class="orange-font">Confronto</h3>
                                <p class="grey-font">Analizziamo più di 100 offerte di partner diversi e selezioniamo la soluzione
                                    giusta per la tua
                                    casa, assicurandoti il
                                    miglior prezzo.</p>
                            </div>
                        </div>
            
                        <div>
                            <g-image class="img-salmone" src="~/assets/images/icon/icon-risparmio.svg" alt="" width="60" />
                            <div>
                                <h3 class="orange-font">Risparmio</h3>
                                <p class="grey-font">Il nostro servizio è gratuito, senza commissioni da parte tua. Ti suggeriamo le
                                    stesse tariffe che le aziende ti
                                    propongono, ma con noi avrai una scelta più consapevole e un risparmio assicurato. </p>
                            </div>
                        </div>
            
                        <div>
                            <g-image class="img-salmone" src="~/assets/images/icon/sicurezza.svg" alt="" width="60" />
                            <div>
                                <h3 class="orange-font">Sicurezza</h3>
                                <p class="grey-font">I tuoi dati sono al sicuro con noi. Tratterremo i tuoi dati sensibili secondo
                                    le norme vigenti, non vendendoli a terzi.</p>
                            </div>
                        </div>
            
                        <div>
                            <g-image class="img-salmone" src="~/assets/images/icon/affidabili.svg" alt="" width="60" />
                            <div>
                                <h3 class="orange-font">Sostegno</h3>
                                <p class="grey-font">Ti verrà affidato un consulente da poter contattare tutte le volte che ne hai
                                    bisogno e un team di assistenza sempre a
                                    tua disposizione. </p>
                            </div>
                        </div>
                        <div>
            
                        </div>
                    </div>
                </div>
            </div>





            <!-- Carosello recensioni 
            <div class="full-container">

                <div class="container recensioni">

                    <h2 class="text-center mb-80">Le recensioni dei clienti che si sono affidati al selezionatore
                    </h2>
                    <ClientOnly>
                        <carousel class="carousel" :perPageCustom="[[768, 1], [1024, 5]]" :paginationEnabled="true">
                            <slide class="carousel-cell carousel-item-recensioni">
                                <div class="salmone-bg">
                                    <div class="Stars" style="--rating: 4.5;" aria-label="Rating of this product is 2.3 out of 5."></div>
                                    <p class="orange-font">Ottimo servizio</p>
                                    <p class="grey-font">Buongiorno, trovo assolutamente positivo il servizio che
                                        Offertasuper.it fornisce
                                        ai
                                        clienti: risposte
                                        veloci</p>
                                </div>
                            </slide>

                            <slide class="carousel-cell carousel-item-recensioni">
                                <div class="salmone-bg">
                                    <div class="Stars" style="--rating: 4.5;" aria-label="Rating of this product is 2.3 out of 5."></div>
                                    <p class="orange-font">Ottimo servizio</p>
                                    <p class="grey-font">Buongiorno, trovo assolutamente positivo il servizio che
                                        Offertasuper.it fornisce
                                        ai
                                        clienti: risposte
                                        veloci</p>
                                </div>
                            </slide>

                            <slide class="carousel-cell carousel-item-recensioni">
                                <div class="salmone-bg">
                                    <div class="Stars" style="--rating: 4.5;" aria-label="Rating of this product is 2.3 out of 5."></div>
                                    <p class="orange-font">Ottimo servizio</p>
                                    <p class="grey-font">Buongiorno, trovo assolutamente positivo il servizio che
                                        Offertasuper.it fornisce
                                        ai
                                        clienti: risposte
                                        veloci</p>
                                </div>
                            </slide>

                            <slide class="carousel-cell carousel-item-recensioni">
                                <div class="salmone-bg">
                                    <div class="Stars" style="--rating: 4.5;" aria-label="Rating of this product is 2.3 out of 5."></div>
                                    <p class="orange-font">Ottimo servizio</p>
                                    <p class="grey-font">Buongiorno, trovo assolutamente positivo il servizio che
                                        Offertasuper.it fornisce
                                        ai
                                        clienti: risposte
                                        veloci</p>
                                </div>
                            </slide>

                            <slide class="carousel-cell carousel-item-recensioni">
                                <div class="salmone-bg">
                                    <div class="Stars" style="--rating: 4.5;" aria-label="Rating of this product is 2.3 out of 5."></div>
                                    <p class="orange-font">Ottimo servizio</p>
                                    <p class="grey-font">Buongiorno, trovo assolutamente positivo il servizio che
                                        Offertasuper.it fornisce
                                        ai
                                        clienti: risposte
                                        veloci</p>
                                </div>
                            </slide>

                            <slide class="carousel-cell carousel-item-recensioni">
                                <div class="salmone-bg">
                                    <div class="Stars" style="--rating: 4.5;" aria-label="Rating of this product is 2.3 out of 5."></div>
                                    <p class="orange-font">Ottimo servizio</p>
                                    <p class="grey-font">Buongiorno, trovo assolutamente positivo il servizio che
                                        Offertasuper.it fornisce
                                        ai
                                        clienti: risposte
                                        veloci</p>
                                </div>
                            </slide>
                        </carousel>
                    </ClientOnly>
                </div>
            </div> -->

            <!-- Loghi -->
            <div class="full-container loghi-area">
                <div class="container">
                    <g-image class="desktop-image" src="~/assets/images/icon/loghi.jpg" alt="" width="1440" />
                    <g-image class="mobile-image" src="~/assets/images/icon/loghi-mobile.jpg" alt="" width="1440" />
                </div>
            </div>




        </div>
    </Layout>
</template>

<page-query>



</page-query>

<script>
import { Pager } from 'gridsome'
import FormLanding from '~/components/FormLanding.vue'

export default {
  components: {
    Pager,
    FormLanding,
   
      Carousel: () =>
        import('vue-carousel')
          .then(m => m.Carousel)
          .catch(),
      Slide: () =>
        import('vue-carousel')
          .then(m => m.Slide)
          .catch(),
    },
  
 
   metaInfo () {
    return {
      title: 'Offerta Super | Selezioniamo le migliori offerte per te',
      meta: [ 
      {
          description: 'Offerta Super è un selezionatore online, in grado di selezionare le migliori offerte per te',
        key: 'Offerta Super',
      },
      {
        key: 'og:title',
        name: 'og:title',
        content: 'Offerta Super',
      },
      {
        key: 'twitter:title',
        name: 'twitter:title',
        content: 'Offerta Super',
      },
      {
        key: 'og:description',
        name: 'og:description',
        content: 'Offerta Super',
      },
      {
        key: 'twitter:description',
        name: 'twitter:description',
        content: 'Offerta Super',
      },
      {
        key: 'og:image',
        name: 'og:image',
        content: '',
      },
    ],
    }
  }
}

</script>

<style lang="scss" soped>
.Stars {
    --percent: calc(var(--rating) / 5 * 100%);
    display: inline-block;
    font-size: var(--star-size);
    font-family: Times;
    line-height: 1
}

.Stars::before {
    content: "★★★★★";
    letter-spacing: 3px;
    background: linear-gradient(90deg, var(--star-background) var(--percent), var(--star-color) var(--percent));
    -webkit-background-clip: text;
    -webkit-text-fill-color: #ffcc00
}

@media screen and (min-width: 1850px) {
    .bg-cielo .VueCarousel-wrapper {
        margin-left: calc((100% - 1440px) / 2);
    }
}

@media screen and (max-width: 1850px) {
    .bg-cielo .VueCarousel-wrapper {
        margin-left: calc(14.2%);
    }
}


.orange-home {

    font-size: 28px;
    text-align: center;
    color: var(--strong-orange);
}


.title-home {
    text-align: center;
}

.grid-five-column {
    display: grid;
    grid-template-columns: 1fr 1fr 1fr 1fr 1fr;
    grid-column-gap: 40px;
    @media screen and (max-width: 768px) {
        grid-template-columns: 1fr 1fr;
    }
}

.grid-two-column {
    display: grid;
    grid-template-columns: 45% 55%;
}

.block-content-img {
    margin: auto;
    display: block;
}

.flex-content-copy {
    display: flex;
    align-content: center;
}

.container-box {
    display: grid;
    grid-template-columns: 1fr;
    grid-column-gap: 20px;

    @media screen and (min-width: 768px) {
        margin-top: 40px;
        grid-template-columns: 1fr 1fr 1fr ;
    }

    .item-box {
        padding: 3px;
    }
}

.carousel-cell {
    width: 100%;
}



@media screen and (max-width: 768px) {
    .bullet-landing {
        display: flex;
        align-items: center;
        margin-bottom: 8px;
        p {
            margin-bottom: 0;
            margin-left: 10px;
            font-size: 19px;
            font-weight: 800;
            color: var(--orang-color);
        }
    }
   .recensioni {
       margin-top: 30px;
   }

   .loghi-area {
       margin-top: 30px;
   }

   .container {
       position: relative;
       overflow: hidden;
   }

   .copy-area {
       margin-top: 0;
   }

   .bg-cielo {
       padding-top: 0;

       h2 {
           margin-bottom: 0;
       }

       .carousel {
           margin-top: 30px;
       }

       .carousel-item-home {
           margin: 0 5px;
       }
   }

   .container-salmone {
       padding-top: 30px;
       padding-bottom: 30px;
       margin-top: 30px;

       .mb-80 {
           margin-bottom: 30px;
           margin-top: 10px;
           text-align: left;
       }

       .grid-five-column {
           grid-template-columns: 1fr;

           &>div {
               display: grid;
               grid-template-columns: 1fr 3fr;

               h3 {
                   margin-top: 0;
                   margin-bottom: 10px;
               }
           }
       }
   }

   .grid-two-column {
       display: grid;
       grid-template-columns: 1fr;
       grid-auto-flow: dense;

       .block-content-img {
           order: 1;
       }

       .flex-content-copy {
           h2 {
               font-size: 22px;
           }

           text-align: center;
           margin-bottom: 20px;
       }
   }

   .img-hero {
       height: 300px;
       position: absolute;
       right: -124px;
       overflow: hidden;

   }

   .home-layout-mobile {
       display: grid;
       grid-template-columns: 2fr 1fr;

       .title-home-mobile {
           margin-top: 80px;
           font-size: 30px;
           color: var(--title-color);
       }

       .p-home-mobile {
           color: var(--grey-body-color);
           font-size: 24px;
       }
   }

   .flex-homeland {
       display: grid;
       grid-template-columns: 1fr 3fr;
       align-items: center;

       .h2-promo {

           font-size: 20px;
           text-align: left;
           font-weight: 800;
       }

       .h3-promo {
           font-size: 17px;
           text-align: left;
           font-weight: 800;
       }
   }

   .container-box .item-box {
       margin-bottom: 30px;
       box-shadow: 0px 10px 20px rgba(116, 116, 116, 0.16);

       .promo {
           position: relative;
           margin-top: -20px;
       }
   }
    .img-hero {
        height: 300px;
        position: absolute;
        right: -124px;
        overflow: hidden;

    }
    .home-layout-mobile {
        display: grid;
        grid-template-columns: 2fr 1fr;

        .title-home-mobile {
            margin-top: 80px;
            font-size: 26px;
            color: var(--title-color);
        }

        .p-home-mobile {
            color: var(--grey-body-color);
            font-size: 23px;
            line-height: 1.3;
        }
    }
  
    .container-box {
        flex-wrap: wrap;
    }

    .carousel-cell {
        width: 100%;
    }
}

.light {
    font-weight: 100;
}

@media screen and (max-width: 768px) and (min-width: 320px) {
    .VueCarousel-inner {
        flex-basis: 230px !important;
    }
}

 
</style>